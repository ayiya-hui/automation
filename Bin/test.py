...
from TestModules.runReport import reportTest
...
tokenDict={'DbPopulator':populatorTest,
           ...
           'Report':reportTest}
not_miss=['PHRuleIncident', 'PHBox', 'SyslogNG']
...
if __name__=='__main__':
    import sys, optparse
    usage="python test.py config_file [-s <ip_address>]"
    parser=optparse.OptionParser()
    parser.add_option('-s', '--source', dest='source', help='using this ip instead of default ip for AO')
    (opts, args)=parser.parse_args()
    if len(args)!=1:
        print usage
        sys.exit()
    file=default_config_file_path+args[0]+'.xml'
    source=''
    if opts.source:
        src=opts.source
    msg=autoTest(file, server=source)
