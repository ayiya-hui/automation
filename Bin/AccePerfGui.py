...
try:
    ...
    sys.stderr.write('AccePerf was unable to start the GUI. Check if wxPython is installed.')
    sys.exit(1)

class AccePerfApp(wx.Frame):
    def __init__(self, parent, config, version):
        wx.Frame.__init__(self, parent, -1, 'AccePerf - Server Performance | Version '+version, size=(690, 710))
        self.runtime_stats={}
        self.error_queue=[]
        self.config=config
        #self.SetIcon(wx.Icon('Accelops.icon').replace('\\', '/'), wx.BITMAP_TYPE_ICO)
        # menus
        file_menu = wx.Menu()
        file_menu.Append(101, '&About', 'About AccePerf')
        wx.EVT_MENU(self, 101, self.on_about)
        file_menu.Append(102, '&Exit', 'Exit AccePerf')
        ...
        self.num_agents_spin.SetValue(self.config.agents)
        self.user_textbox = wx.TextCtrl(panel, -1, 'User')
        self.user_textbox.SetValue(self.config.user)
        self.password_textbox = wx.TextCtrl(panel, -1, 'Password')
        self.password_textbox.SetValue(self.config.password)
        ...
        self.rampup_spin.SetValue(self.config.rampup)
        ...
        if not self.config.name:
            self.name_textbox.SetValue('Accelops Perf Test')
        else:
            self.name_textbox.SetValue(self.config.name)
        ...
        controls_sizer.Add(wx.StaticText(panel, -1, 'User'), 0, wx.TOP|wx.LEFT, 8)
        controls_sizer.Add(self.user_textbox, 0, wx.ALL, 2)
        controls_sizer.Add(wx.StaticText(panel, -1, 'Password'), 0, wx.TOP|wx.LEFT, 8)
        controls_sizer.Add(self.password_textbox, 0, wx.ALL, 2)
        controls_sizer.Add(wx.StaticText(panel, -1, 'Test Name'), 0, wx.TOP|wx.LEFT, 8)
        ...
        self.log_msgs_checkbox.SetValue(self.config.logmsg)
    ...
    def on_about(self, evt):
        ...
        info.SetName('AccePerf')
        info.SetCopyright('This is Accelops Inc. internal Server Performance Tool created by Sha.')
        wx.AboutBox(info)
    ...
    def on_run(self, evt):
        ...
        self.config.agents = self.num_agents_spin.GetValue()
        self.config.interval = self.interval_spin.GetValue() / 1000.0  # convert millisecs to secs
        self.config.rampup = self.rampup_spin.GetValue()
        self.config.duration = self.duration_spin.GetValue()
        self.config.logmsg = self.log_msgs_checkbox.GetValue()
        self.config.name = self.name_textbox.GetValue()
        if self.config.name == 'Test Name':  # user didn't enter a Test Name
            self.config.name = None
        if self.config.name:
            if self.config.outdir:
                self.config.outdir = self.config.outdir + '/' + self.config.name

        # create a load manager
        self.lm = LoadManager(self.config, self.runtime_stats, self.error_queue)

        # load the test cases
        try:
            if '.xml' not in self.config.filename:
                self.config.filename+=".xml"
            cases=XMLHelper.unpickleFile(self.config.filename)
            self.lm.add_cases(cases.testcases)
        except Exception, e:
        ...
        if cases:  # only run if we have valid cases
            ...
            self.stopper = Stopper(self, self.config.duration)
    ...
    def switch_status(self, is_on):
        ...
        if is_on:
            ...
            self.user_textbox.Disable()
            self.password_textbox.Disable()
            ...
            self.user_textbox.Enable()
            self.password_textbox.Enable()
...
def main(config, version):
    app=wx.App(False)
    AccePerfApp(None, config, version)
    app.MainLoop()
