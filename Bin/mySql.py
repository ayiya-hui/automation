...
class dbDataHandler:
    ...
    def getRuleData(self):
        ...
        for line in f.readlines():
            if not 'natural_id' in line and not 'rows' in line:
                item=line.split('|')
                id=item[0].strip()
                naturalId=item[1].strip()
                myRule=rule()
                myRule.attribute['id']=naturalId
                myRule.attribute['group']=self.__getGroupName(id)
                myRule.active=str(item[2]).lower()
                myRule.Name=item[3].strip()
                myRule.Description=item[4].strip()
                eventId=item[10].strip()
                incident_attr=item[11].strip()

                myCustScope=CustomerScope()
                myCustScope.attribute['groupByEachCustomer']=item[5].strip().lower()
                myInclude=Include()
                myInclude.attribute['all']=item[5].strip().lower()
                ...
                type=getType(item[8].strip())
                if type!='NoneType':
                    globalConstr=item[8].strip()
                else:
                    globalConstr=''
                myPattern=self.__getRulePattern(item[6].strip(), RULE_FILTER_ID, id, item[7].strip(), globalConstr)
...
def output(oldData, newData, update=False):
    bad={}
    newAdd={}
    oldmiss={}
