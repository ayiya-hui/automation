...
class HtmlHandler:
    	"""This class is developed to create HTML documents for our
	reporting purpose. But later, we swtich to the Template 	model, rendering it useless. We keep it in the source in 	case there is a new need for HTML documents creating.
	"""
    maxLine = 70
    ...
    def writePreformatted (self, text):
        ...
        self.charCount = HtmlHandler.maxLine
        self.lastWhite = 0
        self.newline = 0
    ...
    def writeWhiteSpace (self):
        if self.lastWhite == 0:
            if self.charCount < HtmlHandler.maxLine:
                self.outfile.write (" ")
...
class HtmlContents:
    ...
    def writeHtml (self, outfile):
        self.writeToHtml (HtmlHandler (outfile))
...
class HtmlElement (HtmlContents):
    ...
    def __init__ (self, aTag, someContents = None, needEnd=1, endLine=0, someDefaultAttributes = [], attributeDict = {}):
        self.tag = aTag
...
class HtmlFormInputText (HtmlElement):
    ...
    def __init__ (self, type, name, value = None, size = None, maxlength = None):
        ...
        HtmlElement.__init__ (self, "INPUT", None, 0, 0, HtmlFormInputText.fieldAttributes, attributes)

        fieldAttributes = [HtmlAttribute ("TYPE", 0),HtmlAttribute ("NAME", 0),HtmlAttribute ("SIZE", 0),HtmlAttribute ("MAXLENGTH", 0),HtmlAttribute ("VALUE")]
...
class HtmlFormTextArea (HtmlPreformattedElement):
    ...
    def __init__ (self, name, cols, rows, contents = None):
        ...
        HtmlPreformattedElement.__init__ (self, "TEXTAREA", contents, 1, 0, HtmlFormTextArea.fieldAttributes, attributes)
        fieldAttributes = [HtmlAttribute ("NAME", 0), HtmlAttribute ("COLS", 0), HtmlAttribute ("ROWS", 0)]
...
class HtmlFormInputCheckBox (HtmlElement):
    ...
    def __init__ (self, type, name, value, contents = None, checked = 0):
        ...
        HtmlElement.__init__ (self, "INPUT", contents, 0, 0, HtmlFormInputCheckBox.fieldAttributes, attributes)
        fieldAttributes = [HtmlAttribute ("TYPE", 0), HtmlAttribute ("NAME", 0), HtmlAttribute ("CHECKED"), HtmlAttribute ("VALUE")]
...
class HtmlFormSelect (HtmlElement):
    ...
    def __init__ (self, name, contents = None, multiple = 0, size = None):
        ...
        if size:
            attributes ["SIZE"] = size
        HtmlElement.__init__ (self, "SELECT", contents, 1, 0, HtmlFormSelect.fieldAttributes, attributes)
        fieldAttributes = [HtmlAttribute ("TYPE", 0),HtmlAttribute ("NAME", 0),HtmlAttribute ("MULTIPLE"),HtmlAttribute ("SIZE", 0)]
...
class HtmlFormOption (HtmlElement):
    ...
    def __init__ (self, contents = None, value = None, selected = 0):
        ...
        HtmlElement.__init__ (self, "OPTION", contents, 0, 1, HtmlFormOption.fieldAttributes, attributes)
        fieldAttributes = [HtmlAttribute ("VALUE"),HtmlAttribute ("SELECTED")]
...
class HtmlFormInputImage (HtmlElement):
    ...
    def __init__ (self, name, source, align = "top"):
        ...
        HtmlElement.__init__ (self, "INPUT", None, 0, 0, HtmlFormInputImage.fieldAttributes, attributes)
        fieldAttributes = [HtmlAttribute ("TYPE", 0),HtmlAttribute ("NAME", 0),HtmlAttribute ("SRC"),HtmlAttribute ("ALIGN")]
...
class HtmlFormInputButton (HtmlElement):
    ...
    def __init__ (self, type, name = None, value = None):
        ...
        if value:
            attributes ["VALUE"] = value
        HtmlElement.__init__ (self, "INPUT", None, 0, 0, HtmlFormInputButton.fieldAttributes, attributes)
        fieldAttributes = [HtmlAttribute ("TYPE", 0), HtmlAttribute ("NAME", 0), HtmlAttribute ("VALUE")]

# Class for table
class HtmlTable (HtmlElement):

    def __init__ (self, content, border=None, name = None, value = None):
        attributes = { "BORDER": border}

        HtmlTagFactory.section("TR", contents)
...
class HtmlTagFactory:
    ...
    def TR (self, contents = None):
        return self.section ("TR", contents)

    def TD (self, contents = None):
        return self.section ("TD", contents)
    ...
    anchorAttributes = [HtmlAttribute ("HREF"),HtmlAttribute ("NAME")]
    ...
    imageAttributes = [HtmlAttribute ("SRC"), HtmlAttribute ("ALIGN"), HtmlAttribute ("ALT"), HtmlAttribute ("ISMAP")]
    ...
    # Table has border attriubte
    tableAttributes = [HtmlAttribute ("BORDER")]

    def TABLE (self, border = None):
        attributes = { "BORDER": border}
        return HtmlElement ("TABLE", None, 1, 0, HtmlTagFactory.tableAttributes, attributes)

    # Form has action and method attributes
    formAttributes = [HtmlAttribute ("ACTION"),HtmlAttribute ("METHOD", 0)]
...
if __name__=='__main__':
    time='2011-3-11 12:35:35'
    myCon=['Module','Total Test','Test Passed']
    file='report.html'
    myHtml=HtmlDocument()
    myHtml.setHtmlTitle('Accelops Automation Test Report')
    myHtml.append(Tag.H1('Test Report'))
    myHtml.append(Tag.HR())
    myHtml.append(Tag.B('Test Running Time: %s Test Category: %s Test Target Build Version: %s' % (time, time, time)))
    wordtag=Tag.TABLE(1)
    wordlist=Tag.TR()
    for word in myCon:
        wordlist.append(Tag.TD(Tag.B(word)))
    wordtag.append(wordlist)
    myHtml.append(wordtag)
    myHtml.append(Tag.A('This', 'http://this'))
    myFile=open(file, 'w')
    myHtml.writeHtml(myFile)
    myFile.close()
    #myHtml.writeHtml(file)
