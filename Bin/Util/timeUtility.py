...
def getTimeNow(second=False):
    ...
    if second:
        ...
        newUtcNow=datetime.datetime(utcnow.year,utcnow.month,utcnow.day,utcnow.hour,utcnow.minute,0,0)
    else:
        newNow=now
        newUtcNow=utcnow
    uNow=int(time.mktime(newNow.timetuple()))
    uUtcNow=int(time.mktime(newUtcNow.timetuple()))

    return newNow, uNow, newUtcNow, uUtcNow

def getTimeNowFormat():
    now=datetime.datetime.now()
    myTime=str(now).replace(" ", "-")
    myTime=myTime.replace(":", "")
    runTime=myTime.split(".")[0]

    return runTime

def getFutureUTime(days):
...
def getUTimeFromString(string):
    ...
    return int(time.mktime(newTime.timetuple())), zone
